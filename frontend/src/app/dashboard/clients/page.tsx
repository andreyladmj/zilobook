    ...
    <div className="p-6 md:p-10 max-w-5xl mx-auto space-y-8 relative pb-24 min-h-screen">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className={clsx("mt-1 font-medium", th.subText)}>Ваші клієнти з історії записів.</p>
        </div>
      </div>

      {loading ? (
        <div className={clsx("text-center py-20 font-semibold", th.subText)}>Завантажуємо клієнтів...</div>
      ) : (
      <div className={clsx("rounded-2xl border shadow-sm overflow-hidden", th.cardBg, th.border)}>
            ...
            <table className="w-full text-left border-collapse">
               <thead>
                  <tr className={clsx("border-b text-xs uppercase tracking-wider font-bold", th.tabBg, th.border, th.subText)}>
                     <th className="p-4 md:p-6">Ім&apos;я</th>
                     <th className="p-4 md:p-6">Телефон</th>
                     <th className="p-4 md:p-6">Візити</th>
                  </tr>
               </thead>
               <tbody>
                  {clients.map(client => (
         ...
         {clients.length === 0 && (
            <div className={clsx("p-10 text-center font-medium", th.subText)}>Клієнтів поки немає. Вони з&apos;являться тут після перших записів.</div>
         )}
      </div>
      )}
    </div>
